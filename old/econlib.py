...
import athena_all.data_reader
from athena_all.sheet import *
...
def findCorr(s, yColName, xColName):
    return s.corr_matrix[yColName][findColumnIndexGivenName(s.df, xColName)]


# arguments: s = Sheet. col = name of column.
# returns: name of column in s's df most correlated with col.
def largestCorr(s, col):
...
# arguments: s = Sheet. col = name of column. topNum = number seeked to return.
# returns: names of topNum columns in s's df most correlated with col.
def largestCorrList(s, col, topNum=3):
    ...
    # we want the (topNum + 1)'th largest because we don't it to count itself
    p = kthLargest(vAbs, topNum + 1)
...
# arguments: s = Sheet. topNum = number seeked for return.
# returns: largest topNum pairwise correlations overall in the dataset (string column names)
def overallLargestCorrs(s, topNum=5):
    ...
    for i in range(0, n):
        for j in range(i, n):
            ...
            v[dfToVectorIndex(n, i, j)] = element
    
    p = kthLargest(v, topNum + 1)
    r = np.zeros(topNum)
    j = 0
...
def reg(s, y, x, clean_data="greedy"):
...
def multiReg(s, y, X, clean_data="greedy"):
...
def fixedEffects(s, y, x, id, year, entity_Effects=False, time_Effects=False, cov_Type='clustered', cluster_Entity=True, clean_data="greedy"):
    
    # prepare data
    v = np.copy(x)
    v = np.append(v, y)
    df = cleanData(s.df, v, clean_data)
...
def logisticRegression(s, y, X, clean_data="greedy"):
...
def ivRegress(s, y, X, Z, exog_regressors=-1, clean_data="greedy", covType="unadjusted", method="2SLS"):
...
def homoskedasticJStatistic(s, y, X, Z, exog_regressors=-1, clean_data="greedy", covType="unadjusted"):
...
# second stage
def test_weak_instruments(s, x, Z, clean_the_data="greedy", covType="unadjusted"):

    # use multiReg because we just need first stage results
    results = multiReg(s, x, Z, clean_data=clean_the_data)
...
def auto_reg(s, y, dates, p, clean_data="greedy"):
...
def AR_with_moving_average(s, var, p, ma, the_dates, clean_data="greedy"):

    # prepare data
    dfClean = cleanData(s.df, var, clean_data)
...
def vector_auto_reg(s, y, dates, p, clean_data="greedy"):
...
# returns PCA object
def principle_component_analysis(s, v, clean_data="greedy"):
...
def poisson_regression(s, endog, exog, clean_data="greedy"):
...
def markov_switching_regime_regression(s, endog, k, exog_vars=-1, clean_data="greedy"):
...
# FUNCTIONS LEFT TO BE WRITTEN
'''
findCov
find_plausible_instruments (given an exogenous one)
find plausible instruments (without an exogenous one)
autocorrelation
autocovariance
'''
# other note
'''
Basically there are multiple ways to write certain things that aren't obvious. Like synonym things.
Like autoregression is the same as time series regression. and vector autoreression is the
same as multivariate time series regression. There are two ways to rectify this: one, have a list of synonyms
and be like also by the way instrument means instrumental variable so maybe use instrument for yours. The second
is just for us to have 3 or 4 different ones for each function and randomly give different ones out to different people and on 
aggregate we'll be money
'''
