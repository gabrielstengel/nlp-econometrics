...
class Athena():
    ...
    def _process_first_query(self, query):
        ...
        return QueryResponse(
            response = f"Succesfully loaded the dataset.\n{query} contained {len(self.dfs)} sheets." if len(self.dfs) > 1 else f"{query} contained only one sheet.",
            result = None,
            computation_time=0,
            flags=['Unkown']
        )
...
def main():
    client = Athena()
    query = input("Hi please enter the file you'd like to work with!\n>>> ")
