...
def main(filename):
    ...
    for amt_ex in amt_exs:
        ex_found = False
        for real_ex in denoted_examples:
            if amt_ex.original_sentence.lower() == real_ex.input:
                examples_formatted.append(Example(amt_ex.rewritten_sentence, real_ex.parse, real_ex.semantics, real_ex.denotation, to_lower=True))
                ex_found = True
        if not ex_found:
