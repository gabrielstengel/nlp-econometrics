...
class MachineLearningMixin:
    ...
    def classify(self, col_y, crossval=True):
        ...
        rdf_cv_results = cross_validate(gnb, X, y, cv=5)
        ...
        utterance += f"\Gaussian Naive Bayes: {svc_acc}\n"
        utterance += f"\tRandom Forest: {svc_acc}\n"
        utterance += f"Thus we recomend using {_name_best_method()}. You can search for optimized hyperparameters using Athena as well."
