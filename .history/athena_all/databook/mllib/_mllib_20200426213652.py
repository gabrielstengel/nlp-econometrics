...
class MachineLearningMixin:
    ...
    def kmeans(self, k=5):
        ...
        kmeans = KMeans(n_clusters=k, random_state=0).fit(np.array(numeric_columns[0]))
        utterance = f"K-Means successfully converged after {kmeans.n_iter} iterations. Only used numeric columns."
        utterance += f"\nHere are the cluster {k} centers. The values are presented in the column order:\n"
        utterance += f"\t\t"
        for clust in kmeans.cluster_centers:
            utterance += f"\t\t"
