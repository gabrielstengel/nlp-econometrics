...
from athena_all.databook.sheet.econlib._econlib_fmap import econlib_fmap
...
class EconLibMixin: 
    ...
    def findCorr(self, yColName, xColName):
        return self.corr_matrix[yColName][self.findColumnIndexGivenName(self.df, xColName)]
    ...
    def largestCorr(self, col):
        ...
        return self.findVariableName(m, champ)
    ...
    def largestCorrList(self, col, num_return=3):
        ...
        for i in range(0, n):
            c = vAbs[i]
            if c >= p and c < 1:
                returnVector.append(self.findVariableName(m, i))
    ...
    def overallLargestCorrs(self, num_return=5):
        ...
        for i in range(0, len(r)):
            ...
            s.append(self.findVariableName(m, t))
    ...
    def reg(self, y, x, clean_data="greedy"):
        ...
        v = np.array(x)
        v = np.append(v, y)
        dfClean = self.cleanData(self, v, clean_data)
        X = dfClean[x]
        y = dfClean[y]
        X = sm.add_constant(X)
    ...
    def fixedEffects(self, y, x, id, year, entity_Effects=False, time_Effects=False, cov_Type='clustered', cluster_Entity=True, clean_data="greedy"):
        ...
        v = np.copy(x)
        v = np.append(v, y)
        df = self.cleanData(self, v, clean_data)
    ...
    # Check for sufficient first-stage identification 
        ...
        dfClean = self.cleanData(df, v, clean_data)
        X = dfClean[X]
        length = len(X)
        Z = dfClean[Z]
        y = dfClean[y]
    ...
    def AR_with_moving_average(self, var, p, ma, the_dates, clean_data="greedy"):
        ...
        # prepare data
        dfClean = self.cleanData(self, var, clean_data)
    ...
    def markov_switching_regime_regression(self, endog, k, exog_vars=-1, clean_data="greedy"):
        ...
        if exog_vars != -1:
            ...
            dfClean = self.cleanData(self.df, v, clean_data)
            endog = dfClean[endog]
