...
from athena_all.databook import QueryResult
...
class EconLibMixin: 
    ...
    def findCorr(self, yColName, xColName):
        ...
        utterance = "The correlation between " + str(yColName) + " and " + str(xColName) + " is " + str(corr) + "."
    ...
    def largestCorr(self, col):
        ...
        utterance = "The variable most correlated with " + str(col) + " is " + str(best_var) + "."
    ...
    def largestCorrList(self, col, num_return=3):
        ...
        utterance = "The " + str(num_return) + " variables most correlated with " + str(col) + " are "
    ...
    def overallLargestCorrs(self, num_return=5, index=0):
        ...
        utterance = "Here are the " + str(num_return) + " most correlative pairwise relationships in the dataset\n"
    ...
    def reg(self, y, x, clean_data="greedy"):
        ...
        utterance = "Here are the results of a linear regression of " + str(y_arg) + " on " + str(x_arg) + ".\n\n" 
    ...
    def multiReg(self, y, X, clean_data="greedy"):
        ...
        utterance = "Here are the results of a multivariate linear regression of " + str(y_arg) + " on " + str(X_arg) + ".\n\n" 
        utterance = utterance + str(results.summary())
    ...
    def fixedEffects(self, y, x, id, year, entity_Effects=False, time_Effects=False, cov_Type='clustered', cluster_Entity=True, clean_data="greedy"):
        ...
        if type(x) != str:
            utterance = "ERROR: Multiple independent regressor approach not yet implemented."
            return utterance
        ...
        utterance = "Here are the results of a fixed effects regression of " + str(y) + " on " + str(x)
        utterance = utterance + ", using " + str(year) + " as the time dimension and " + str(id) + " as the id dimension.\n\n"
        utterance = utterance + str(mod.fit(cov_type=cov_Type, cluster_entity=cluster_Entity))

        return QueryResult(mod.fit(cov_type=cov_Type, cluster_entity=cluster_Entity), utterance)
    ...
    # Check for sufficient first-stage identification 
        ...
        utterance = utterance + " on endogenous covariates " + str(X_args) + ", using " + str(Z_args) + " as instruments.\n\n"
    ...
    def homoskedasticJStatistic(self, y, X, Z, exog_regressors=-1, clean_data="greedy", covType="unadjusted"):
        ...
        utterance = "\nThe homoskedastic Wald j-statistic test output in a regression of " + str(arg_y) + " on endogenous covariates " + str(arg_X) 
        utterance = utterance + ", using " + str(arg_Z) + " as instruments is the following:\n\n"
    ...
    def test_weak_instruments(self, x, Z, clean_the_data="greedy", covType="unadjusted"):
        ...
        if type(x) != str:
            utterance = "Multiple endogenous regressors not yet implemented for this test."
            return QueryResult(None, utterance)
        ...
        # want F > 10
        f = results.fvalue
        utterance = "The F-value in a regression with endogenous covariate " + str(x_arg) + " and instruments " + str(Z_arg)
        utterance = utterance + " is " + str(f) + ".\n Typically, we want F > 10 to have reliably strong estimates in the first stage."
    ...
    def find_instruments(self, y, X, exog_Z, candidates):
        ...
        utterance = "This is the most inclusive group of jointly valid instruments among those"
        utterance = utterance + " suggested in a regression of " + str(y) + " on " + str(X) + " with " + str(exog_Z)
    ...
    def augmented_dicky_fuller_test(self, var, max_lag=-1):
        ...
        utterance = utterance + "The null hypothesis is that the process has a unit root. The lower the p-value, "
    ...
    def granger_p_value(self, results, dep, ind):
        ...
        utterance = "The p-value of the Granger causality test is " + str(r.p_value) + ".\n"
    ...
    def analyze_lags(self, cols, time, preferred_criterion='aic', min_lag=1,max_lag=8):
        ...
        utterance = ""
        for i in range(max_lag - min_lag + 1):
            utterance = utterance + "AIC (" + str(i + min_lag) + " lags): " + str(aic[i]) + "\n"
        
        utterance = utterance + "\n\n"

        for i in range(max_lag - min_lag + 1):
            utterance = utterance + "BIC (" + str(i + min_lag) + " lags): " + str(bic[i]) + "\n"
        
        utterance = utterance + "\n\n"

        x = np.argsort(aic)
        champ = aic[x[0]]
        utterance = utterance + "Using AIC, here are the estimated proportional probabilities, using the best as a reference:"
        utterance = utterance + "\n"
        for i in range(max_lag - min_lag + 1):
            utterance = utterance + str(i + min_lag) + " lags: " + str(find_prob_given_AIC(champ, aic[i])) + "\n"
        
        optimal = self.find_optimal_lag_length(cols, time, min_lag=min_lag, max_lag=max_lag, criterion=preferred_criterion).get_denotation()
    ...
    def find_optimal_lag_length(self, cols, time, min_lag=1,max_lag=8, criterion='aic'):
        ...
        utterance = "The optimal lag length according to the " + str(criterion) + " criterion is "
    ...
    def poisson_regression(self, endog, exog, clean_data="greedy"):
        ...
        utterance = "Here are the results of a Poisson regression with endogenous variables "
        utterance = utterance + str(arg_endog) + " and exogenous variables " + str(arg_exog) + ".\n"
    ...
    def markov_switching_regime_regression(self, endog, k, exog_vars=-1, clean_data="greedy"):
        ...
        fit = mr.fit()
        utterance = "Here are the results of a dynamic Markov regression with endogenous variable " + str(arg_endog)
    ...
    def choose_among_regression_models(self, sets_of_vars, criterion='aic', clean_data='greedy'):
        ...
        utterance = "The best-fitting model according to the " + str(criterion) + " criterion among these ones:\n"
    ...
    def get_econlib_fmap(self):
        ...
        (['median'],
        'findMedian', self.findMedian),

        #(['correlation'],
        # 'findCorr', findCorr),

        (['largest correlation', 'biggest correlation'],
        'largestCorr', self.largestCorr),

        (['largest correlations', 'biggest correlations'],
        'largestCorrList', self.largestCorrList),

        (['linear regression', 'reg', 'regression'],
        'reg', self.reg),

        (['multivariate regression', 'multivariable reg', 'reg', 'regress', 'linear regression', 'regression'],
        'multiReg', self.multiReg),

        (['fixed effects', 'panel', 'longitudinal'],
        'fixedEffects', self.fixedEffects),

        (['logistic', 'logit', 'binary', 'logistic reg'],
        'logisticRegression', self.logisticRegression),

        (['marginal effects', 'margins'],
        'logisticMarginalEffects', self.logisticMarginalEffects),

        (['instrument', 'iv', 'instrumental variable', 'reg', 'regression', 'instrumental regression', 'regress'],
        'ivRegress', self.ivRegress),

        (['exogeneity', 'j-statistic', 'instrument' 'valid', 'j stat', 'homoskedastic j stat'],
        'homoskedasticJStatistic', self.homoskedasticJStatistic),

        (['weak', 'strong', 'strength' 'instrument', 'relevant', 'relevance'],
        'test_weak_instruments', self.test_weak_instruments),

        (['time series', 'autoregression', 'AR'],
        'auto_reg', self.auto_reg),

        (['time series', 'autoregression', 'AR', 'stationarity', 'test'],
        'augmented_dicky_fuller_test', self.augmented_dicky_fuller_test),

        (['time series', 'vector', 'multivariate autoregression', 'VAR'],
        'vector_auto_reg', self.vector_auto_reg),
        ...
        'granger_causality_test', self.granger_causality_test),
        ]
