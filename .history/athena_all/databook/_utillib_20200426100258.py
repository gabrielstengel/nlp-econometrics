...
class UtilLibMixin:
    ...
    def help(self, functions=None):
        if functions is None or functions == "generic":
            utterance = ""
            utterance += "\nFor help about a specific topic, just ask about it. (e.g. 'How do I run a regression?')"
            return QueryResult(utterance, utterance)
        ...
        for idx, f in enumerate(functions):
            ...
            if f == "largestCorr":
                q = (
                    q
                    + "\t\tTo find the variable most correlated with a variable, try:\n"
                )
            ...
            if f == "overallLargestCorrs":
                ...
                q = (
                    q
                    + "\t\twhat are the most significant relationships in the dataset\n\n"
                )
            ...
            if f == "logisticMarginalEffects":
                q = (
                    q
                    + "\t\tTo calculate the marginal effects of a logistic model, try:\n"
                )
            ...
            if f == "test_weak_instruments":
                ...
                q = (
                    q
                    + "\t\ttest the strength of _instrument_var_1_ and _instrument_var_2_"
                )
            ...
            if f == "analyze_lags":
                q = (
                    q
                    + "\t\tTo decide how many lags to use in a time series model, try:\n"
                )
                q = (
                    q
            ...
            if f == "print_PCA_wrapper":
                ...
                q = (
                    q
                    + "\t\trun principle component analysis on _var_1_ and _var_2_\n\n"
                )
