...
class Grammar:
    ...
    def add_rule(self, rule):
        ...
        if rule.add_all_permutations:
            for perm in list(permutations(range(len(rule.rhs))))[1:]:
                new_rhs = tuple([rule.rhs[pi] for pi in perm])
                new_sems = lambda sems: [sems[pi] + str(new_rhs) for pi in perm]
                new_sems = lambda sems: rule.sem(
                    [sems[pi] + str(new_rhs) for pi in perm]
                )
                new_rule = Rule(rule.lhs, new_rhs, new_sems)
