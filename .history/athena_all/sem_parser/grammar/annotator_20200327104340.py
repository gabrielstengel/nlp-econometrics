...
class FunctionAnnotator(Annotator):
    def __init__(self, syns_to_names):
        assert len(syns_to_names[0]) == 2, "Function map should only contain synonyms and semantics."
    ...
    def get_similair_function(self, query):
        ...
        func_sim = np.argmax([np.max(np.array([lev.ratio(term, query) for term in fmap[0]])) for fmap in self.syns_to_names])
