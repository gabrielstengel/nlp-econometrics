...
class Grammar:
    ...
    def _orderings(self):
        all_perms = list(permutations(range(len(self.rhs))))[1:]
    ...
    def add_rule(self, rule):
        ...
        if rule.add_all_permutations:
            for perm, perm_i in self._orderings():
                new_rhs = tuple([rule.rhs[pi] for pi in perm])
                new_sems = lambda sems: [sems[pi] for pi in perm_i]
                new_rule = Rule(
                    rule.lhs, new_rhs, lambda sems, perm=perm: rule.sem(new_sems(sems)),
                )
