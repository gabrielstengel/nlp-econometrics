...
class Grammar:
    ...
    def add_rule(self, rule, add_all_permutations=False):
        if self.contains_rule(rule):
            if DEBUG:
                print(f"WARNING:: TRYING TO ADD DUPLICATE RULE: {str(rule)}")
            return

        if add_all_permutations:
            for perm in permutations(range(len(rule.rhs)))[1:]:
                new_rhs = [rule.rhs[pi] for pi in perm]
                new_sems = lambda sems: tuple([sems[pi] for pi in perm])
                new_rule = Rule(rule.lhs, new_rhs, lambda sems: rule.sem(new_sems(sems)))
