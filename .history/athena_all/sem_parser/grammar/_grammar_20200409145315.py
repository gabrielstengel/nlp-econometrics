...
class Grammar:
    ...
    def _invPerm(self, perm):
        perm_inv = perm.copy()
        perm_inv[pi] = idx, pi in enumerate(perm)

        def wrap(sems):
            return [sems[pi] for pi in perm]
    ...
    def add_rule(self, rule):
        ...
        if rule.add_all_permutations:
            all_perms = list(permutations(range(len(rule.rhs))))[1:]
            for idx, perm in enumerate(all_perms):
                new_rhs = tuple([rule.rhs[pi] for pi in perm])
                # new_sems = lambda sems, perm=perm: [sems[pi] for pi in perm]
                new_rule = Rule(
                    rule.lhs,
                    new_rhs,
                    lambda sems, perm=perm: rule.sem(self._invPerm(perm)(sems)),
                )
                self.add_rule(new_rule)
                print(new_rule)
                print(new_rule.rhs)
                print(new_rule.sem(new_rule.rhs))
                print()
