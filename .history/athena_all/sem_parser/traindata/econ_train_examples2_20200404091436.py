...
class trainingExamples:
    ...
    def get_train_examples(self):
        ...
        logit_reg_1 = db.summarizeLogisticRegression("dummy", dep_vars_1).denotations
        logit_reg_2 = db.summarizeLogisticRegression("dummy", dep_vars_2).denotations
        logit_reg_3 = db.summarizeLogisticRegression("dummy", dep_vars_3).denotations
        logit_reg_4 = db.summarizeLogisticRegression("dummy", dep_vars_4).denotations
        ...
        """
        auto_reg_1 = db.auto_reg("stinger", "year", 3).denotation
        auto_reg_2 = db.auto_reg("matic", "year", 4).denotation
        auto_reg_3 = db.auto_reg("auto", "year", 1).denotation
        auto_reg_4 = db.auto_reg("gdp", "year", 2).denotation
        """
        ...
        analyze_lags_3 = db.analyze_lags(dep_vars_3, "year").denotation
        analyze_lags_4 = db.analyze_lags("stinger", "year").denotation

        find_optimal_lag_length_1 = db.find_optimal_lag_length(
            dep_vars_1, "year", min_lag=3, max_lag=5
        ).denotation
        find_optimal_lag_length_2 = db.find_optimal_lag_length(
            dep_vars_2, "year"
        ).denotation
        find_optimal_lag_length_3 = db.find_optimal_lag_length(
            dep_vars_3, "year", min_lag=2, max_lag=11
        ).denotation
        ...
        denoted_examples = [
            ...
            Example(
                input="Find the j-statistic instrumental regression of AGE on EASY, MATIC, ENTERQ, YEAR, and ANIMAL with GDP, PANEL_YEAR, DUMMY, BEAST, TANK and STINGER as instrumentals",
                semantics=("ivRegress", "age", dep_vars_4, instruments_4),
                to_lower=True,
                denotation=iv_fit_4,
            ),
            Example(
                input="what's the j-stat in a regression GDP, EASY, AUTO, and DUMMY as instrumental variables to reg BEDWARDS on STINGER, MATIC, and BEAST",
                semantics=("ivRegress", "bedwards", dep_vars_1, instruments_1),
                to_lower=True,
                denotation=iv_fit_1,
            ),
            Example(
                input="Find the exogeneity statistic in a regression of STINGER, MATIC, BEAST, and AUTO as instrumetns to reg of GDP on BEDWARDS, TANK, and DUMMY",
                semantics=("ivRegress", "gdp", dep_vars_2, instruments_2),
                to_lower=True,
                denotation=iv_fit_2,
            ),
            Example(
                input="Homoskedastic j stat in reg of STINGER, PERSON_ID, DUMMY, YEAR, and AGE as instrumental variables regress BEAST on GDP, EASY, AUTO, and MATIC",
                semantics=("ivRegress", "beast", dep_vars_3, instruments_3),
                to_lower=True,
                denotation=iv_fit_3,
            ),
            Example(
                input="find j stat in reg of GDP, PANEL_YEAR, DUMMY, BEAST, TANK and STINGER as instrumental variables regress AGE on EASY, MATIC, ENTERQ, YEAR, and ANIMAL",
                semantics=("ivRegress", "age", dep_vars_4, instruments_4),
                to_lower=True,
                denotation=iv_fit_4,
            ),
            ...
            # --------------------------------------------------------------------------------------------------------------- #
            # auto_reg: proper grammar
            # WAIT UNTIL WRAPPER PIECE IS SORTED OUT
            # auto_reg_1 = db.auto_reg("stinger", "year", 3)
            # auto_reg_2 = db.auto_reg("matic", "year", 4)
            # auto_reg_3 = db.auto_reg("auto", "year", 1)
            # auto_reg_4 = db.auto_reg("gdp", "year", 2)
            # Example(
            #     input="run a time series regression on stinger with year as dates with 3 lags",
            #     semantics=("auto_reg", "stinger", "year", 3),
            #     to_lower=True,
            #     denotation=auto_reg_1,
            # ),
            # Example(
            #     input="univariate autoregression on matic with 4 lags with year as dates",
            #     semantics=("auto_reg", "matic", "year", 4),
            #     to_lower=True,
            #     denotation=auto_reg_2,
            # ),
            # Example(
            #     input="univariate autoregression on auto with 3 lags with year as dates",
            #     semantics=("auto_reg", "auto", "year", 1),
            #     to_lower=True,
            #     denotation=auto_reg_3,
            # ),
            # --------------------------------------------------------------------------------------------------------------- #
            ...
            # --------------------------------------------------------------------------------------------------------------- #
            # vector_auto_reg: proper grammar
            # --------------------------------------------------------------------------------------------------------------- #
            # summarize_VAR: proper grammar
            # --------------------------------------------------------------------------------------------------------------- #
            # granger_p_value: proper grammar
            # --------------------------------------------------------------------------------------------------------------- #
            # granger_causality_test: proper grammar
            # --------------------------------------------------------------------------------------------------------------- #
            # analyze_lags: proper grammar
            # analyze_lags_1 = db.analyze_lags("matic", "year")
            # analyze_lags_2 = db.analyze_lags(dep_vars_2, "year")
            # analyze_lags_3 = db.analyze_lags(dep_vars_3, "year")
            # analyze_lags_4 = db.analyze_lags("stinger", "year")
            # dep_vars_2 = ["bedwards", "tank", "dummy"]
            # dep_vars_3 = ["gdp", "easy", "auto", "matic"]
            Example(
                input="how many lags should I use in a time series regression through year on matic?",
                semantics=("analyze_lags", "matic"),
                to_lower=True,
            ...
            Example(
                ...
                semantics=("analyze_lags", dep_vars_2),
                to_lower=True,
                denotation=analyze_lags_2,
            ),
            Example(
                input="number of lags in vector auto regression on gdp, easy, auto, and matic that uses year as the time?",
                semantics=("analyze_lags", dep_vars_3),
                to_lower=True,
                denotation=analyze_lags_3,
            ),
            Example(
                input="how many lagged values in autoregression on stinger that uses year as the time?",
                semantics=("analyze_lags", "stinger"),
                to_lower=True,
                denotation=analyze_lags_4,
            ),
            # --------------------------------------------------------------------------------------------------------------- #
            # find_optimal_lag_length: proper grammar
            # find_optimal_lag_length_1 = db.find_optimal_lag_length(dep_vars_1, "year", min_lag=3, max_lag=5).denotation
            # find_optimal_lag_length_2 = db.find_optimal_lag_length(dep_vars_2, "year").denotation
            # find_optimal_lag_length_3 = db.find_optimal_lag_length(dep_vars_3, "year", min_lag=2, max_lag=11).denotation
            # find_optimal_lag_length_4 = db.find_optimal_lag_length(dep_vars_4, "year").denotation
            # dep_vars_1 = ["stinger", "matic", "beast"]
            # dep_vars_2 = ["bedwards", "tank", "dummy"]
            # dep_vars_3 = ["gdp", "easy", "auto", "matic"]
            # dep_vars_4 = ["easy", "matic", "enterq", "year", "animal"]
            Example(
                input="best number of lags between 3 and 5 lags in time series regression on stinger, matic, and beast that uses year as the time?",
                semantics=("find_optimal_lag_length", dep_vars_1),
                to_lower=True,
                denotation=find_optimal_lag_length_1,
            ),
            Example(
                input="optimal number of lags in time series regression on bedwards, tank, and dummy that uses year as the time?",
                semantics=("find_optimal_lag_length", dep_vars_2),
                to_lower=True,
                denotation=find_optimal_lag_length_2,
            ),
            Example(
                input="Using year as the the time, what is the optimal number of lags between 2 and 11 in time series regression on gdp, easy, auto, and matic?",
                semantics=("find_optimal_lag_length", dep_vars_3),
                to_lower=True,
                denotation=find_optimal_lag_length_3,
            ),
            Example(
                input="optimal lag number in time series regression on easy, matic, enterq, year, and animal that uses year as the time?",
                semantics=("find_optimal_lag_length", dep_vars_4),
                to_lower=True,
                denotation=find_optimal_lag_length_4,
            ),
            # --------------------------------------------------------------------------------------------------------------- #
            # principle_component_analysis: proper grammar
            # --------------------------------------------------------------------------------------------------------------- #
